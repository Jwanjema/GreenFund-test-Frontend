                        ...
                        <LineChart data={formattedChartData} margin={{ top: 5, right: 10, left: -40, bottom: 5 }}>
                             <Tooltip
                                ...
                                formatter={(value, name, props) => {
                                     // Customize tooltip based on card title
...
function Dashboard() {
  ...
  // --- UPDATED: Added cropSuggestions to state ---
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // --- UPDATED: Initialize suggestions ---
      let suggestionsData = { unique_suggestion_count: 0, recent_suggestions: [] };

      try {
        // --- UPDATED: Fetch suggestions too ---
        const [farmsRes, badgesRes, emissionsRes, suggestionsRes] = await Promise.allSettled([
            apiClient.get('/farms/'),
            apiClient.get('/badges/me/count'),
            apiClient.get('/activities/emissions/weekly'),
            apiClient.get('/soil/suggestions/summary') // <-- Fetch suggestions
        ]);

        // Process farms and activities
        if (farmsRes.status === 'fulfilled') {
            farmCount = farmsRes.value.data.length;
            if (farmCount > 0) {
                try { /* Fetch activities */
                    const firstFarmId = farmsRes.value.data[0].id;
                    const activityRes = await apiClient.get(`/activities/farm/${firstFarmId}`);
                    activities = activityRes.data.slice(0, 3);
                } catch (e) { console.error("Failed activities fetch:", e); }
            }
        ...
        // --- UPDATED: Set all states ---
        setDashboardData({
            totalFarms: farmCount,
            badgeCount: badgeCount,
            weeklyEmissions: emissionsData,
            cropSuggestions: suggestionsData // <-- Set suggestions state
        });
   ...
   const getEmissionsTrend = () => { /* ... */
    ...
    // --- Create Trend String for Suggestions ---
